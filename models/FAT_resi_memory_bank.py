...
class PatchMemoryBank(nn.Module):
    def __init__(self, max_size, patch_size, feature_dim, device=None):
        ...
        """
        super(PatchMemoryBank, self).__init__()  # <--- 修改2: 初始化父类
        self.max_size = max_size
        self.patch_size = patch_size
        self.feature_dim = feature_dim
        self.device = device
        self.register_buffer('patches', torch.zeros((max_size, feature_dim), device=device))
        self.ptr = 0
    ...
    def update(self, new_patches):
        ...
        # 确保新进来的数据和 memory bank 在同一个设备上
        if new_patches_flat.device != self.patches.device:
            new_patches_flat = new_patches_flat.to(self.patches.device)
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # ==================== [Version 2: Memory Bank 组件] ====================
        self.mem_size = getattr(configs, 'memory_size', 20)
        self.top_k = getattr(configs, 'top_k', 5)

        # 1. 初始化 Memory Bank
        self.patch_memory_bank = PatchMemoryBank(
            max_size=self.mem_size,
            patch_size=configs.seq_len,
            feature_dim=configs.d_model,
            device=None
        )

        # 2. Local Memory MLP (用于处理检索出的特征)
        ...
        # 3. [用户改进] Memory Fusion Gate (门控融合)
        # 输入: [Query特征, Memory特征] 拼接后的维度
        # 输出: 2个权重值 (Softmax归一化)
    ...
    def _compute_local_memory(self, query_features, features_to_save):
        """
        计算 Local Memory 并通过门控融合
        query_features: [B, N, S, D] -> 受损特征 (rebuild_embed)
        features_to_save: [B*N, S, D] -> 纯净特征 (clean_enc_out)
        """
        # 维度调整: query_features 进来时可能是 [B, N, S, D]，为了检索需要 flatten
        B, N, S, D = query_features.shape
        query_flat_view = query_features.reshape(B * N, S, D)

        # 1. 准备用于检索和存储的向量 (Mean Pooling)
        query_vec = query_flat_view.mean(dim=1)  # [B*N, D]
        save_vec = features_to_save.mean(dim=1)  # [B*N, D]

        # 2. 检索 Top-K
        retrieved_patches, _ = self.patch_memory_bank.retrieve(query_vec, top_k=self.top_k)

        # 3. 更新 Memory Bank (存真)
        self.patch_memory_bank.update(save_vec)

        # 4. 处理检索到的特征 (MLP + Mean Aggregation)
        local_memory = self.local_memory_mlp(retrieved_patches)  # [B*N, Top_K, D]
        local_memory = local_memory.mean(dim=1)  # [B*N, D]

        # 5. 扩展回序列维度，以便与 query_features 融合
        # [B*N, 1, D] -> [B, N, 1, D] -> [B, N, S, D]
        local_memory = local_memory.view(B, N, 1, D).expand(-1, -1, S, -1)

        # ==================== [用户改进: 门控融合] ====================
        # 拼接: [B, N, S, 2*D]
        combined = torch.cat([query_features, local_memory], dim=-1)

        # 计算权重: [B, N, S, 2]
        gate_weights = self.memory_fusion_gate(combined)

        # 加权求和
        # w0 * Query(原始) + w1 * Memory(历史)
        enhanced_features = (
                gate_weights[..., 0:1] * query_features +
                gate_weights[..., 1:2] * local_memory
        )
    ...
    def pretrain_residual(self, batch_x):
        ...
        clean_enc_out = enc_out[:bs * n_vars]  # [B*N, S, D] 用于存
        ...
        implicit_feat_enhanced = self._compute_local_memory(
            query_features=rebuild_embed,
            features_to_save=clean_enc_out
        )

        implicit_feat = implicit_feat_enhanced
