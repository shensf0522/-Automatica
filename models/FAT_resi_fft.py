...
class Model(nn.Module):
    ...
    def pretrain_residual(self, batch_x):
        ...
        # 1) 显式结构（趋势） + 残差
        x_season, x_resi = self.DFT_series_decomp(batch_x)

        # 2) RevIN 归一化（完全沿用 FAT 的逻辑）
        z = self.revin_layer_encoder(x_resi, 'norm')  # [B, S, N]
        ...
        loss_rb = self.mse(x_resi, pred_r)
    ...
    def forecast(self, x):
        ...
        # bs, seq_len, n_vars = x.shape
        # # 处理残差
        # x_season, x_resi = self.DFT_series_decomp(x)
        # z = self.revin_layer_encoder(x_resi, 'norm')
        # x = z
        # x = x.permute(0, 2, 1)
        # if self.configs.forcastMode == "freq":
        #     x, _ = self.KnowledgeGuide_encoder(x)  # B, N, D
        ...
        # x_season = x_season.reshape(-1, seq_len, 1)
        # x_season = self.season_linear(x_season).unsqueeze(-1)
        # x = x + x_season
