...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # 施加门控机制来融合信息
        # 1. 显式特征投影层: 将 (B, N, S, 1) -> (B, N, S, D)
        self.season_embed_proj = nn.Linear(1, configs.d_model)
    ...
    def pretrain_residual(self, batch_x):
        ...
        r_all = torch.cat([x_normed, r_positives], dim=0)
        ...
        # [Step 1] 维度对齐
        # x_season 从 (bs, seq_len, n_vars) 变换为 (bs, n_vars, seq_len, 1)
        season_feat = x_season.permute(0, 2, 1).unsqueeze(-1)

        # [Step 2] 特征投影 & 防作弊处理
        # 映射到 d_model 维度
        season_emb = self.season_embed_proj(season_feat)  # (bs, n_vars, seq_len, d_model)
        # Critical: 加入 Dropout 防止模型直接抄袭显式特征
        season_emb = self.season_dropout(season_emb)

        # [Step 3] 计算门控系数 (Gating)
        # 这里的逻辑是：让 rebuild_embed 自己“告诉”模型它是否自信
        # gate_score 越大，表示越需要引入 season_emb
        gate_score = self.fusion_gate(rebuild_embed)  # (bs, n_vars, seq_len, 1)
        # [Step 4] 特征融合
        # 融合公式：H_final = (1 - \alpha) * H_implicit + \alpha * H_explicit
        # 这样当 alpha 接近 0 时，保留原始重建；alpha 接近 1 时，引入显式知识
        fused_embed = (1 - gate_score) * rebuild_embed + gate_score * season_emb

        # 可选：经过 LayerNorm 稳定分布
