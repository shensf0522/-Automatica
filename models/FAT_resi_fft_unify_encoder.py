...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # #趋势项的预测
        # self.trend_projection = Flatten_Head(configs.seq_len, 8, configs.pred_len, head_dropout=configs.head_dropout)
        # 在enc_out处融合趋势的编码
        ...
        # 2. 门控网络: 输入是隐式特征(D维度)，输出是门控系数(1维度)
        # 逻辑：根据当前的隐式表征质量，判断需要多少显式特征的介入
        self.fusion_gate = nn.Sequential(
            nn.Linear(configs.d_model, configs.d_model // 4),
            nn.Tanh(),  # Tanh 激活有助于处理特征的相对强弱
            nn.Linear(configs.d_model // 4, 1),
            nn.Sigmoid()  # 输出 0~1 之间的系数
        )
        # 3. 防作弊 Dropout:
        # 高 Dropout 率 (如 0.2~0.5) 迫使模型将其视为“辅助参考”而非“标准答案”
        self.season_dropout = nn.Dropout(configs.struct_dropout)
        # 4. 最终融合后的归一化 (可选，有助于训练稳定)
    ...
    def pretrain_residual(self, batch_x):
        ...
        season_flat = x_season.permute(0, 2, 1).reshape(-1, seq_len)
        ...
        # Knowledge_guide
        r_reformed, _ = self.KnowledgeGuide_encoder(x_normed)  # B, N, D
        ...
        r_all = torch.cat([x_normed, r_positives, season_flat], dim=0)

        # Encoderr
        enc_out = self.enc_embedding(r_all.unsqueeze(-1))
        enc_out_all, _ = self.encoder(enc_out)

        # 切分出各个部分的东西
        # [Step 4] 切分输出 (Split)
        # 我们需要算出各个部分的长度来进行切分
        len_orig = x_normed.shape[0]  # bs * n_vars
        len_pos = r_positives.shape[0]  # bs * n_vars * positive_nums
        len_season = season_flat.shape[0]  # bs * n_vars
        enc_out, enc_pos, enc_season = torch.split(enc_out_all, [len_orig, len_pos, len_season], dim=0)

        # 使用不包含season的来重建序列
        enc_for_cl = torch.cat([enc_out, enc_pos], dim=0)

        # Contrastive Learning
        s_enc_out = self.cl_projection(enc_for_cl)
        ...
        # rebuild origin
        positive_enc_out = enc_pos.reshape(bs * n_vars, self.configs.positive_nums, -1)
        ...
        # enc_season (显式): (bs*n, seq, d) -> (bs, n, seq, d)
        season_deep_feat = enc_season.reshape(bs, n_vars, seq_len, -1)

        # 2. 门控计算 (Stop Gradient)
        gate_input = rebuild_embed.detach()
        gate_score = self.fusion_gate(gate_input)  # (bs, n, seq, 1)

        # 3. 防作弊 Dropout
        season_deep_feat = self.season_dropout(season_deep_feat)

        # 4. 融合
        fused_embed = (1 - gate_score) * rebuild_embed + gate_score * season_deep_feat
        fused_embed = self.fusion_norm(fused_embed)
        ...
        # [Step 5] 最终预测 (输入替换为 fused_embed)
        # pred_x = self.head_pretrain(fused_embed)  ### MODIFY
