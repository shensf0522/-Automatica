...
class DFT_series_decomp(nn.Module):
    ...
    def forward(self, x):
        ...
        x_season = torch.fft.irfft(xf, dim=1, norm='ortho')
        x_resi = x - x_season
        return x_season, x_resi
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # ===========================================================================
        # ==================== 新增：DFBP 频率平衡预训练模块（完全保留原有代码，增量添加） ====================
        self.use_dfbp = getattr(configs, 'use_dfbp', False)  # 新增配置，默认False，不影响原有逻辑
        if self.use_dfbp:
            # 1. 小波包分解的工具（如果没有安装pywt，可以先用细粒度FFT分解替代）
            # 你可以先安装：pip install pywt，用来做细粒度的高频分解
            import pywt
            self.wp_decomp = pywt.WaveletPacket
            self.wavelet = 'db4'  # 常用的小波基

            # 2. 自适应频率门控：用来区分有效高频和噪声
            self.freq_gate = nn.Sequential(
                nn.Linear(configs.seq_len, configs.seq_len // 2),
                nn.ReLU(),
                nn.Linear(configs.seq_len // 2, 1),
                nn.Sigmoid()
            )

            # 3. 三个分支的重建头：分别处理低频、有效高频、噪声
            # 低频沿用你原来的head，高频我们加新的，噪声加去噪头
            self.head_high_eff = Flatten_Head(configs.seq_len, configs.d_model, configs.seq_len,
                                              head_dropout=configs.head_dropout)
            self.head_high_noise = Flatten_Head(configs.seq_len, configs.d_model, configs.seq_len,
                                                head_dropout=configs.head_dropout)

            # 4. 平衡损失的权重
            self.dfbp_alpha = 1.2  # 有效高频的损失权重，>1 增强高频学习
            self.dfbp_beta = 0.8  # 噪声的损失权重
            self.dfbp_gamma = 0.1  # 平衡正则项的权重
    ...
    def pretrain_residual(self, batch_x):
        ...
        x_season, x_resi = self.DFT_series_decomp(z) # 从z中分离出显示项
        ...
        explicit_feat = x_season.permute(0, 2, 1).unsqueeze(-1).reshape(bs * n_vars, seq_len, 1)
        ...
        if self.use_dfbp:
            # 1. 处理GPU张量：临时转到CPU（保留梯度无关，仅分解用）
            x_cpu = x_normed.detach().cpu().numpy()  # [bs*n_vars, seq_len]
            device = x_normed.device
            bs_nvars, seq_len = x_cpu.shape

            # 2. 初始化小波包，统一分解逻辑（maxlevel=2，适配所有节点命名）
            x_low_list = []
            x_high_list = []

            # 预定义小波包分解的节点路径（maxlevel=2时的标准路径）
            # level 1: 'a'(低频), 'd'(高频)
            # level 2: 'aa'(低频), 'ad', 'da', 'dd'(均为高频)
            wavelet = pywt.Wavelet(self.wavelet)

            for i in range(bs_nvars):
                # 对每个样本做小波包分解（修正初始化+索引方式）
                wp = pywt.WaveletPacket(data=x_cpu[i], wavelet=wavelet, mode='symmetric', maxlevel=2)

                # 提取低频分量（level 2的aa节点）
                try:
                    x_low = wp['aa'].data  # level 2 低频
                except KeyError:
                    # 兼容不同小波基的节点命名，兜底用level 1的a节点
                    x_low = wp['a'].data
                # 统一低频分量长度（和原序列对齐）
                x_low = np.pad(x_low, (0, seq_len - len(x_low)), 'constant')[:seq_len]

                # 提取所有高频分量（level 2的ad/da/dd节点）
                high_components = []
                for path in ['ad', 'da', 'dd']:
                    try:
                        high_comp = wp[path].data
                        # 统一长度，和原序列对齐
                        high_comp = np.pad(high_comp, (0, seq_len - len(high_comp)), 'constant')[:seq_len]
                        high_components.append(high_comp)
                    except KeyError:
                        # 节点不存在时补0
                        high_components.append(np.zeros_like(x_low))

                # 合并高频分量（按维度拼接，保证和低频同长度）
                x_high = np.stack(high_components, axis=0).mean(axis=0)  # 平均合并，避免维度爆炸

                x_low_list.append(x_low)
                x_high_list.append(x_high)

            # 转回GPU张量（恢复原设备）
            x_low = torch.tensor(np.array(x_low_list), dtype=torch.float32, device=device)
            x_high = torch.tensor(np.array(x_high_list), dtype=torch.float32, device=device)

            # 3. 子频率归一化（逻辑不变，修复维度匹配）
            x_low = (x_low - x_low.mean(dim=-1, keepdim=True)) / (x_low.std(dim=-1, keepdim=True) + 1e-5)
            x_high = (x_high - x_high.mean(dim=-1, keepdim=True)) / (x_high.std(dim=-1, keepdim=True) + 1e-5)

            # 4. 自适应门控（修正维度：确保gate_weight和x_high维度匹配）
            gate_weight = self.freq_gate(x_high)  # [bs*n_vars, 1]
            # 用软门控替代硬分割（避免空张量问题）
            gate_weight = gate_weight.squeeze(-1)  # [bs*n_vars]
            x_high_eff = x_high * gate_weight.unsqueeze(-1)  # 有效高频（加权）
            x_high_noise = x_high * (1 - gate_weight).unsqueeze(-1)  # 噪声分量（加权）

            # 5. 分别重建（修复空张量问题，无需判断len>0）
            # 低频重建（用原有head）
            rec_low = self.head_pretrain(self.enc_embedding(x_low.unsqueeze(-1)))
            loss_low = torch.nn.functional.mse_loss(rec_low.squeeze(-1), x_low)

            # 有效高频重建（新head）
            rec_high_eff = self.head_high_eff(self.enc_embedding(x_high_eff.unsqueeze(-1)))
            loss_high_eff = torch.nn.functional.mse_loss(rec_high_eff.squeeze(-1), x_high_eff)

            # 噪声分量去噪重建（加微小扰动）
            noise_aug = x_high_noise + 0.01 * torch.randn_like(x_high_noise)
            rec_high_noise = self.head_high_noise(self.enc_embedding(noise_aug.unsqueeze(-1)))
            loss_high_noise = torch.nn.functional.mse_loss(rec_high_noise.squeeze(-1), x_high_noise)

            # 6. 平衡正则项（修复空张量问题）
            losses = torch.tensor([loss_low.item(), loss_high_eff.item(), loss_high_noise.item()], device=device)
            balance_loss = torch.var(losses)

            # 7. 合并损失（和原有逻辑兼容）
            dfbp_loss = loss_low + self.dfbp_alpha * loss_high_eff + self.dfbp_beta * loss_high_noise + self.dfbp_gamma * balance_loss
            loss = self.awl(loss_cl, loss_rb) + 0.1 * dfbp_loss
        else:
