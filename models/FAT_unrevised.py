...
from utils.augmentations import augment_positive
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if self.task_name == "clf":
            self.head_clf = Flatten_Head(configs.input_len, configs.d_model, configs.nums_class, head_dropout=configs.head_dropout)
        else:
    ...
    def pretrainWithContrast(self, batch_x):
        ...
        """
        对batch_x 进行norm
        x_raw_normed: 原始序列的时域norm版本
        x_freq_reformed: 原始序列的频域修正版本
        """
        x_raw_normed = self.revin_layer_encoder(batch_x, 'norm')
        x_raw_normed = x_raw_normed.permute(0, 2, 1)
        sim_matrix = FFT_sim(x_raw_normed) # (b*n, b*n)
        x_raw_normed = x_raw_normed.reshape(-1, seq_len) # (b*n, s)
        # 生成相似度矩阵用于挑选hard-negative
        negative_index = torch.topk(sim_matrix, k = self.configs.negative_nums, dim=1).indices
        # fre norm
        x_freq_reformed, _ = self.fre_norm_encoder(x_raw_normed)  # B, N, D
        # positive_cases
        x_positives = augment_positive(x_freq_reformed, self.configs.mask_rate, self.configs.lm, k=self.configs.positive_nums)
        x_positives = x_positives.reshape(-1, seq_len)
        ## ( (K+1)*b*n, s)
        # x_all = torch.cat([x_raw_normed,  x_positives], dim=0)
        # Cl_learning
        ## mask only positive samples
        mask = get_mask(x_positives, "geometric", self.configs.mask_rate, self.configs.lm, seq_len).squeeze()
        x_positive_masked = x_positives * mask
        ## encode all sampels
        enc_out = self.enc_embedding(torch.cat([x_raw_normed,  x_positive_masked], dim=0).unsqueeze(-1))
        enc_out, _ = self.encoder(enc_out)
        # bs * n_vars * （k+1） , d_model
        ############# cl loss ##############
        '''
        让时域embedding和频域reformed进行相似
        使用完整的原始序列（x_raw），对Frequency-reformed + noise的正样本、以及hard-nagetive进行相似度计算
        '''
        s_enc_out = self.cl_projection(enc_out)
        s_enc_out = F.normalize(s_enc_out, dim=1)
        s_q = s_enc_out[: bs * n_vars] # 原始序列作为 q
        s_k = s_enc_out[bs * n_vars:].reshape(bs * n_vars, self.configs.positive_nums, -1) # postive and negative as k
        if self.labels_cl is None:
            self.labels_cl = generate_CLLabels(x_raw_normed, self.configs.positive_nums, self.configs.negative_nums)
        ...
        log_similarity_matrix = self.log_softmax(similarity_matrix)
        loss_cl = self.kl(log_similarity_matrix, self.labels_cl)
        # if self.labels_cl.size(0) != bs:
        #     print("error")
        #     current_size = self.labels_cl.size(0)
        #     pad_size = bs - current_size
        #     self.labels_cl = F.pad(self.labels_cl, (0, 0, 0, pad_size), value=0.5)

        ## inference_loss  
        # diff_ = p_enc_out[bs * n_vars :2 * bs * n_vars] - p_enc_out[:bs * n_vars]
        # diff_ = self.infer_projection(diff_)
        # w_real = self.head_Inference_Re(diff_)
        # w_imag = self.head_Inference_Img(diff_)
        # w_freq = torch.cat([w_freq.real, w_freq.imag], dim=-1)
        # w = torch.cat([w_real, w_imag], dim=-1)
        # loss_infer = self.mse(w_freq,w)
        ######### rebuild loss ##############
        '''
        使用频域reformed对原始序列进行重建。
        '''
        ...
        return loss, loss_cl, loss_rb, 0, None, None, None
    ...
    def forecast(self, x):
        ...
        x = z
        x = x.permute(0, 2, 1)
        if self.configs.forcastMode == "freq": # 是否适用频域知识库进行修正, config.use_freqKB
