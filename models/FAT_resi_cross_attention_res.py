...
class Model(nn.Module):
    ...
    def pretrain_residual(self, batch_x):
        ...
        x_resi = x_resi.permute(0, 2, 1)
        sim_matrix = FFT_sim(x_resi)  # (b*n, b*n)   不同样本，不同特征之间的相似性
        x_resi = x_resi.reshape(-1, seq_len)  # (b*n, s)
        negative_index = torch.topk(sim_matrix, k=self.configs.negative_nums, dim=1).indices
        # Knowledge_guide
        r_reformed, _ = self.KnowledgeGuide_encoder(x_resi)  # B, N, D
        ...
        r_all = torch.cat([x_resi, r_positives], dim=0)
        ...
        if self.labels_cl is None:
            self.labels_cl = generate_CLLabels(x_resi, self.configs.positive_nums, self.configs.negative_nums)
