...
class RevIN(nn.Module):
    ...
    def _get_statistics(self, x):
        ...
        #关注的是一个样本中的一个特征在时间维度上的均值和方差,各特征独立
...
class MDM(nn.Module):
    def __init__(self, input_shape, k=3, c=2, layernorm=True):
        ...
        if self.k > 0:
            ...
            self.avg_pools = nn.ModuleList([nn.AvgPool1d(kernel_size=k, stride=k) for k in self.k_list])  # kernel_size表示一次看k个步长，一次滑动k个，并且一个窗口内进行平均化
    ...
    def forward(self, x):
        if self.layernorm:
            x = self.norm(torch.flatten(x, 1, -1)).reshape(x.shape)  # 这里的norm是一个把特征维度和时间维度特征做了展平，然后对展平后的每一个维度做跨样本的归一化，也就是某个特征（通道）在跨样本下的均值和方差去做归一化，然后再把结果返回为了原始的数据特征
        if self.k == 0:
            return x
...
class DDI(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.n_history * self.patch, self.input_shape[0], self.patch):  #这里的input_shape[0]是时间序列的长度，是初始化的时候添加的
            # input [batch_size, feature_num, self.n_history * patch]
            input = output[:, :, i - self.n_history * self.patch: i]  # 表示从第一块的内容开始，从output中取出来，是处理后的内容，而不是原始的X
            # input [batch_size, feature_num, self.n_history * patch]
            input = self.norm1(torch.flatten(input, 1, -1)).reshape(input.shape)   # 做局部的BN
            # aggregation
            # [batch_size, feature_num, patch]
            input = F.gelu(self.agg(input))  # self.n_history * patch -> patch （最后一个维度上的变化）
            input = self.dropout_t(input)
            # input [batch_size, feature_num, patch]
            # input = torch.squeeze(input, dim=-1)
            tmp = input + x[:, :, i: i + self.patch] # 将上一个patch线性变换的结果和当前patch的真实值进行相加

            res = tmp # 暂存历史的结果和真实的结果（仅做时间维度

            # [batch_size, feature_num, patch]
            if self.alpha > 0.0:
                tmp = self.norm2(torch.flatten(tmp, 1, -1)).reshape(tmp.shape) # 局部BN
                tmp = torch.transpose(tmp, 1, 2)  # 转正成 [batch_size, patch, feature_num]
                tmp = self.fc_block(tmp)  # feature_num -> ff_dim -> feature_num
