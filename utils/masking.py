...
def geom_noise_mask_single(L, masking_ratio, lm,  max_length):
    ...
    # 一种 mask 概率的处理
    # 每个掩蔽序列停止的概率，反映了掩蔽序列的平均长度。
    p_m = 1 / lm  # probability of each masking sequence stopping. parameter of geometric distribution.
    # 未掩蔽序列停止的概率，确保在整体比例中满足掩蔽比例。
    p_u = p_m * masking_ratio / (1 - masking_ratio)  # probability of each unmasked sequence stopping. parameter of geometric distribution.
    p = [p_m, p_u]
    # 随机决定初始状态，state 为 0 表示掩蔽，1 表示未掩蔽。初始状态根据 masking_ratio 确定。   True为1，False为0
    # state = initial_state  # state 0 means masking, 1 means not masking
